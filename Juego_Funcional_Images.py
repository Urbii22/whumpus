...
class WumpusGUI:
    def __init__(self, root):
        ...
        # Mapeo de estados a listas de imágenes
        self.tile_to_images = {
            BLANCO: [self.images[BLANCO]],
            AGENTE: [self.images[AGENTE]],
            HOYO: [self.images[HOYO]],
            WUMPUS: [self.images[WUMPUS]],
            ORO: [self.images[ORO]],
            HEDOR: [self.images[HEDOR]],
            BRISA: [self.images[BRISA]],
            HEDOR_ORO: [self.images[HEDOR], self.images[ORO]],
            BRISA_ORO: [self.images[BRISA], self.images[ORO]],
            BRISA_HEDOR: [self.images[BRISA], self.images[HEDOR]],
            BRISA_HEDOR_ORO: [self.images[BRISA], self.images[HEDOR], self.images[ORO]]
        }
    ...
    def cargar_imagenes(self):
        """
        Carga las imágenes necesarias para cada tipo de casilla.
        Redimensiona todas las imágenes al tamaño de las casillas para una apariencia uniforme.
        Asegúrate de que las imágenes estén en la carpeta 'ImagenesCasillasWumpus' y tengan los nombres correctos.
        """
        try:
            # Tamaño deseado para cada imagen (ancho, alto)
            desired_size = (self.cell_size, self.cell_size)

            # Cargar y redimensionar las imágenes utilizando Pillow
            pil_images = {
                BLANCO: Image.open('ImagenesCasillasWumpus/CasillaVacia.png').resize(desired_size, Image.ANTIALIAS),
                AGENTE: Image.open('ImagenesCasillasWumpus/CasillaAgente.png').resize(desired_size, Image.ANTIALIAS),
                HOYO: Image.open('ImagenesCasillasWumpus/CasillaHueco.png').resize(desired_size, Image.ANTIALIAS),
                WUMPUS: Image.open('ImagenesCasillasWumpus/CasillaWumpus.png').resize(desired_size, Image.ANTIALIAS),
                ORO: Image.open('ImagenesCasillasWumpus/CasillaOro.png').resize(desired_size, Image.ANTIALIAS),
                HEDOR: Image.open('ImagenesCasillasWumpus/CasillaHedor.png').resize(desired_size, Image.ANTIALIAS),
                BRISA: Image.open('ImagenesCasillasWumpus/CasillaBrisa.png').resize(desired_size, Image.ANTIALIAS)
            }

            # Convertir las imágenes de Pillow a PhotoImage de Tkinter
            self.images = {tile: ImageTk.PhotoImage(img) for tile, img in pil_images.items()}

        except Exception as e:
            messagebox.showerror("Error de Carga de Imágenes", f"No se pudieron cargar o redimensionar las imágenes. Asegúrate de que estén en la carpeta 'ImagenesCasillasWumpus', con los nombres correctos y sean archivos de imagen válidos.\n\nDetalles del Error:\n{e}")
            self.root.destroy()
    ...
    def draw_board(self):
        ...
        for i in range(self.tamano):
            for j in range(self.tamano):
                ...
                tile = matrix[i][j]
                images_to_draw = self.tile_to_images.get(tile, [])

                for img in images_to_draw:
